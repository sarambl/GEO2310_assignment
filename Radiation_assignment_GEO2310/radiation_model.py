...
class RadiationModel(Grid):
    ...
    def runRadiationModel(self, n_t, dt, t0=0):
        ...
        for i_t in np.arange(n_t - 1):
            ...
            # TODO EXERCISE 6:
            # Hint: DeltaF_SH and DeltaF_LW_ total are both energy fluxes
            SH_heating_rate[:, i_t] = 0.0 # Exercise 6
            ...
            # Update surface temperature:
            T_s[i_t + 1] = T_s[i_t] + \
                           ((1 - self.albedo) * SW_down_surf + LW_down['Surface'][i_t] - sigma * T_s[i_t] ** 4 - SH0) \
                           * 3600 / Cs
            # Update temperature:
            T[:, i_t + 1] = T[:, i_t] + (heating_rates_total[:, i_t] + SH_heating_rate[:, i_t]) * dt
        # Make dictionary of output:
        output = {}
        output['SW_down'] = SW_down  # flux density down
        output['DeltaF_SW_down'] = DeltaF_SW_down
        output['mu'] = mu # mu
        output['SW_heating_rates'] = SW_heating_rates # SW heating rates
        output['LW_up'] = LW_up # LW flux up
        output['LW_down'] = LW_down # LW flux down
        output['LW_heating_rates'] = LW_heating_rates
        output['T'] = T
        output['T_s'] = T_s
        output['DeltaF_LW_total'] = DeltaF_LW_total
        output['DeltaF_LW_down'] = DeltaF_LW_down
        output['DeltaF_LW_up'] = DeltaF_LW_up
        output['SH_heating_rate'] =SH_heating_rate
        output['heating_rates_total'] = heating_rates_total
        return output
    ...
    def init_model_vars(self, n_t):
        ...
        """
        # Decleration of variables for run. Do not change!
        SW_down = {}
        LW_down = {}
        LW_up = {}
